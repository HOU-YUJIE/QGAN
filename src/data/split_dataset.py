#!/usr/bin/env python3
"""
Split original dataset into train and test sets by Label,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Split dataset 8:2 by category")
    parser.add_argument(
        "-i",
        "--input",
    ...
    parser.add_argument(
        "-tr",
        "--train-output",
        default=os.path.join("data", "processed", "selected_features_train.csv"),
        help="Training set output path",
    )
    parser.add_argument(
        "-te",
        "--test-output",
        default=os.path.join("data", "processed", "selected_features_test.csv"),
